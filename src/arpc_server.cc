...
Server::Server(ell_Ipv4Addr &localAddr)
...
// void Server::default_Message(Connector *conn, Setting *setting) {
// void Server::default_Message(Connector *conn) {}

void Server::defaultClose(Connector *conn, Setting *setting) {
    conn->close();
...
void Server::setdefaultMessage(MessageCall call) {
    message = std::move(call);
}

void Server::defaultAccept(Connector *conn, Setting *setting) {
    while (1) {
        ...
        client_conn->CloseCallBack =
            std::bind(&Server::defaultClose, this, client_conn, setting);
...
void Server::acceptInit() {
    ...
    acceptor->ReadCallBack = std::bind(&Server::defaultAccept, this, copy, setting);
